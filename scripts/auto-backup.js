...
// 配置
const config = {
  githubRepo: process.env.GITHUB_BACKUP_REPO, // 格式: owner/repo
...
async function autoBackup() {
  try {
    ...
    if (!config.githubRepo || !config.githubToken) {
      console.error('缺少GitHub配置: GITHUB_BACKUP_REPO 或 GITHUB_TOKEN')
      process.exit(1)
    }

    // 验证Fine-grained Token格式
    if (!config.githubToken.startsWith('github_pat_')) {
      console.error('请使用GitHub Fine-grained Personal Access Token (以 github_pat_ 开头)')
      process.exit(1)
    }
...
async function syncToGitHub(mdFile, dateStr, totalLinks) {
  try {
    ...
    try {
      ...
      throw new Error('Git未安装或不在PATH中，请先安装Git')
    }
