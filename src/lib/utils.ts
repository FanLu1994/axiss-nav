...
export function getAdminProfile(): AdminProfile {
  ...
  const email = process.env.ADMIN_EMAIL?.trim() || `${username}@local.axiss-nav`;
...
export function verifyAdminCredentials(username: string, password: string): boolean {
  if (!isAdminAuthConfigured()) {
    throw new Error("管理员认证尚未配置，请检查环境变量");
  }
...
export function generateToken(payload: JWTPayload): string {
  return jwt.sign(payload, getJwtSecret(), { expiresIn: "180d" }); // 半年（180天）
}
